...
class Processor(object):
    ...
    def __init__(self, dataset, model, args):
        ...
        # self.prev_slot_list = [torch.zeros(1, SLOT_DIM)]
        # self.prev_intent_list = [torch.zeros(1, INTENT_DIM)]
        ...
        for epoch in range(0, self.__dataset.num_epoch):
            ...
            for data_detail_batch in tqdm(dataloader, ncols=50):
                ...
                for turn_index,turn in enumerate(data_detail_batch):
                    ...
                    if turn_index!=0:
                        local_var = Variable(torch.LongTensor(local_list))
                    else:
                        local_var = Variable(torch.zeros(self.__batch_size, self.__max_column_len, self.__slot_num))
                    ...
                    #
                    #
                    # slot_list = self.__dataset.slot_alphabet.get_instance(nested_slot)
                    # if turn_index == 0:
                    #     digit_intent_list = Evaluator.max_freq_predict(nested_intent)
                    #     text_intent_list = self.__dataset.intent_alphabet.get_instance(digit_intent_list)
                    #
                    # for sen_index, [sen_intent, sen_slot, sen_text] in enumerate(zip(digit_intent_list, slot_list, text_batch)):
                    #     if turn_index == 0:
                    #         curr_local = [[] for _ in range(self.__max_column_len)]
                    #     else:
                    #         curr_local = local_list[sen_index]
                    #
                    #     curr_intent = self.__dataset.intent_alphabet.get_instance(sen_intent)
                    ...
                    #     # print("len {}. {}".format(len(sen_slot), len(sen_text)))
                    ...
                    #             if len(curr_local[slot_index]) < self.__slot_num:
                    ...
                    #         if len(single_local) < self.__slot_num:
                    #             padded_local = single_local
                    #             padded_local = padded_local.extend([0]* (self.__slot_num-len(single_local)) )
                    #
                    # # batch padding
                    # for sen_local_index, sen_local in enumerate(local_list):
                    #     batch_padding = [ [0]*self.__slot_num] * self.__max_column_len
                    #     if(len(sen_local)==0):
    ...
    def prediction(model, dataset, mode, batch_size):
        ...
        for data_detail_batch in tqdm(dataloader, ncols=50):
            ...
            for turn_index, turn in enumerate(data_detail_batch):
                ...
                pred_slot.extend(curr_slot_alphabet.get_instance(nested_slot))
