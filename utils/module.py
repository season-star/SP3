...
class ModelManager(nn.Module):
    ...
    def __init__(self, args, num_word, num_slot0, num_slot1, num_slot2, num_intent, num_kb, num_history, mem_sentence_size):
        ...
        self.__num_slot0 = num_slot0
        self.__num_slot1 = num_slot1
        self.__num_slot2 = num_slot2
        ...
        self.__kb_num_vocab = num_word+ self.__sum_num+ num_intent+num_kb
        self.__his_num_vocab = num_word + self.__sum_num + num_intent + num_kb + num_history
        ...
        self.__slot_decoder0 = LSTMDecoder(
            ...
            output_dim=self.__max_num_slot,
            dropout_rate=self.__args.dropout_rate,
        ...
        self.__lk_slot_linear = nn.Linear(self.__sum_num,self.__args.local_knowledge_hidden_dim)
        ...
        self.__lk_attention_layer = QKVAttention(
            query_dim=self.__args.encoder_hidden_dim,
            key_dim=self.__args.encoder_hidden_dim,
            value_dim=self.__args.encoder_hidden_dim,
            hidden_dim=self.__args.attention_hidden_dim,
        ...
        # self.__linear_list.append(nn.Linear(self.__args.encoder_hidden_dim,self.__args.encoder_hidden_dim))  #lstm_hiddens
    ...
    def show_summary(self):
        ...
        print('\tnumber of slot:                            {} {} {};'.format(self.__num_slot0,self.__num_slot1, self.__num_slot2))
    ...
    def forward(self, text, seq_lens, text_triple=None, kb=None, dial_id=None, turn_id=None, history=None, n_predicts=None, forced_slot=None, forced_intent=None, local=None, turn_index=None, if_train=None):
        cat_init_hiddens = nn.Parameter(
                torch.zeros(sum(seq_lens), 1),
                requires_grad=True
            )
        ...
        if (self.__add_mem):
            ...
            # hiddens = torch.cat([lstm_hiddens, attention_hiddens],dim=1)
            hiddens = lstm_hiddens
        else:
        ...
        if n_predicts is None:
            return F.log_softmax(pred_slot, dim=1), F.log_softmax(pred_intent, dim=1), slot_idx.cpu().data.numpy().tolist(), intent_idx.cpu().data.numpy().tolist()
        else:
...
class LSTMDecoder(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, dropout_rate, mem_dim, lk_dim, his_dim, add_mem=False, embedding_dim=None, extra_dim=None, max_output_dim = None):
        ...
        if(add_mem):
            self.__input_dim = input_dim #第三个mem_dim其实是history_dim
        else:
            self.__input_dim = input_dim
...
class MemN2N(nn.Module):
    ...
    #将text作为hidden输入
    def load_mem(self, story, hidden, seq_len=None,p=None):
